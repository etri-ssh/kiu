...
public  class Mag_Heading {
    ...
    //가속도 ax,ay,az을 통한 Roll,Pitch (사용 frame 확인 필요할 수 있음)
...
//        a_pitch = Math.atan2(ax,Math.sqrt(ay*ay+az*az));
        a_roll = Math.atan2(ay,Math.sqrt(ax*ax+az*az));
        a_pitch = Math.atan2(ax,-az);
...
//        Yaw_m = Math.atan2((-nom),den);
        ...
        den = Mag[1]*Math.cos(a_roll)+
                Mag[0]*Math.sin(a_pitch)*Math.sin(a_roll) -
                Mag[2]*Math.cos(a_pitch)*Math.sin(a_roll);


        nom = -Mag[0]*Math.cos(a_pitch)-Mag[2]*Math.sin(a_pitch);


        Yaw_m = -90*d2r-Math.atan2((-nom),den);
